...
- Approved library: `Midnight in Bonetown`, `A Tomb of Twins`, `Born Into Black Nights`, and `The Old Blood` have reached approved built-in status after central source/canon/rules/licence/adversarial QA and required machine validation.
- The Old Blood: completed a final 88-page source audit, added exact Sewer/Catacomb/Ruins/Secret Dungeon/Flooded Shrine registers, recovered the full 20-name resident table, fixed four missing named residents, verified Blood Rite/item/native-stat dependencies, migrated the importer to v2, passed central review, and recorded formal approval for Shadowdark levels 1-2. Exact Shadowdark mechanics remain adapter-owned rather than being reclassified as CC0.
...
- Non-fantasy queue: verified additional open sci-fi candidates including `The Return of the XBRC Terror`, `Cascading Failure`, `Derelict Transdimensional Anomaly`, `Nautilus of Time`, and `The Horror of Station XK-629`; post-apocalyptic `Pharmacy Run` is CC0 and queued for DOS-module extraction.
