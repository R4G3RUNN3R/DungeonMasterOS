...
- V1 verification: final clean-room `npm ci`, build, typecheck, 29/29 regression tests, and `npm audit --omit=dev --audit-level=high` pass after the entitlement correction. The four remaining moderate audit notices are development-tool-only dependencies and are excluded from the production runtime audit.
