...
- V1 verification: final build, typecheck, 30/30 regression tests, and `npm audit --omit=dev --audit-level=high` pass after the entitlement correction and client-payload check. The four remaining moderate audit notices are development-tool-only dependencies and are excluded from the production runtime audit.
