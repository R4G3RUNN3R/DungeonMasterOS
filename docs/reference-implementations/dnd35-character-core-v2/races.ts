// Reference implementation only. Mechanical summaries are deliberately concise.
...
export type ConditionalRacialModifier = {
  target: string;
  value: number;
  bonusType: "racial" | "dodge" | "size" | "untyped";
  condition: string;
};
...
export type Dnd35RaceDefinition = {
  ...
  /** Stable flags consumed by mechanics/adapters rather than parsed from prose. */
  rules: {
...
export const CORE_RACES: Record<CoreRaceId, Dnd35RaceDefinition> = {
  human: race({
    ...
    bonusLanguages: "any_non_secret",
    senses: [],
    rules: {
      extraFeatAtLevelOne: 1,
      extraSkillPointsAtLevelOne: 4,
      extraSkillPointsPerAdditionalLevel: 1,
    },
    weaponProficiencies: [],
    weaponFamiliarities: [],
    skillModifiers: [],
  ...
  dwarf: race({
    ...
    rules: {
      armorDoesNotReduceLandSpeed: true,
      loadDoesNotReduceLandSpeed: true,
    },
    weaponProficiencies: [],
    weaponFamiliarities: ["dwarven waraxe", "dwarven urgrosh"],
    skillModifiers: [
    ...
    spellLikeAbilities: [],
    specialTraits: ["Stonecunning", "Stability", "Dwarven weapon familiarity"],
  }),
  ...
  elf: race({
    ...
    rules: {
      automaticSecretDoorSearch: true,
      magicalSleepImmunity: true,
    },
    weaponProficiencies: [
      "longsword",
      "rapier",
    ...
    conditionalModifiers: [
      { target: "save:any", value: 2, bonusType: "racial", condition: "against enchantment spells and effects" },
    ],
    spellLikeAbilities: [],
    specialTraits: ["Immunity to magical sleep", "Automatic Search opportunity for nearby secret/concealed doors"],
  }),
  ...
  gnome: race({
    ...
    senses: [{ type: "low_light", multiplier: 2 }],
    rules: { illusionSaveDcBonus: 1 },
    weaponProficiencies: [],
    weaponFamiliarities: ["gnome hooked hammer"],
    skillModifiers: [
      { skillId: "listen", value: 2, bonusType: "racial" },
    ...
    spellLikeAbilities: [
      {
        id: "gnome:speak-with-animals",
        name: "Speak with Animals",
        usesPerDay: 1,
        casterLevel: 1,
        spellLevel: 1,
        requiresMinimumCharisma: 10,
        restriction: "burrowing mammals only",
      },
      {
        id: "gnome:dancing-lights",
        name: "Dancing Lights",
        usesPerDay: 1,
        casterLevel: 1,
        spellLevel: 0,
  ...
  "half-elf": race({
    ...
    spellLikeAbilities: [],
    specialTraits: ["Elven blood", "Immunity to magical sleep"],
  }),
  ...
  halfling: race({
    ...
    conditionalModifiers: [
      ...
      { target: "save:any", value: 2, bonusType: "racial", condition: "against fear; stacks with the halfling all-saves bonus" },
...
export function applyRacialAbilityAdjustments(
  base: Dnd35AbilityScores,
  raceDefinition: Dnd35RaceDefinition,
): Dnd35AbilityScores {
  const result = { ...base };
  for (const [ability, adjustment] of Object.entries(raceDefinition.abilityAdjustments)) {
    const key = ability as Dnd35Ability;
