...
- The fresh live crawl used for the d20srd.org gate genuinely called `assertClosureExhaustive` before diffing (see `scripts/run-srd-manifest-acceptance-scan.ts`'s `runD20srdCompletenessGate`, which calls it immediately after the crawl and before any diff logic runs). This gate is fail-closed by construction, not merely by this run's luck: `assertClosureExhaustive` throws unconditionally whenever `failures.length > 0`, so the diff logic below it is only ever reachable when the crawl had zero failures — the real logged `"failures: 0"` line is consistent with, not separate proof of, that guarantee.

## Reproducibility

The real scan, both gates, and the scan-revision stamp are all one re-runnable script: `scripts/run-srd-manifest-acceptance-scan.ts` (`node --import tsx scripts/run-srd-manifest-acceptance-scan.ts`, from the repo root, targeting the real dev database). It performs the exact sequence documented above — discovery scan, then both gates (stamping only proceeds if both pass), then `recordSourceScanRevision` — so this report's real output is independently reproducible, not a one-off unrecorded snippet.
