...
export type Dnd35PrerequisiteResult = {
  qualified: boolean;
  failures: string[];
};
...
function evaluateOne(requirement: Dnd35Prerequisite, state: Dnd35FeatQualificationState): Dnd35PrerequisiteResult {
  ...
  switch (requirement.kind) {
    ...
    case "feat": {
      ...
      const selectedMatch = state.featSelections?.some(
        (selection) => selection.featId === requirement.featId && (requirement.parameter === undefined || selection.parameter === requirement.parameter),
      ) ?? false;
      return simpleMatch || selectedMatch
        ? pass()
        : fail(`Feat ${requirement.featId}${requirement.parameter ? ` (${requirement.parameter})` : ""} required.`);
    }
    case "class_level": {
    ...
    case "race": {
      return state.races?.includes(requirement.raceId) ? pass() : fail(`Race ${requirement.raceId} required.`);
    }
    case "alignment": {
      return state.alignment && requirement.allowed.includes(state.alignment)
        ? pass()
        : fail(`Alignment must be one of: ${requirement.allowed.join(", ")}.`);
    }
    case "proficiency": {
      return state.proficiencies?.includes(requirement.proficiencyId)
        ? pass()
        : fail(`Proficiency ${requirement.proficiencyId} required.`);
    }
    case "spellcasting":
    case "special": {
      const rule = requirement.kind === "spellcasting" ? requirement.requirement : requirement.rule;
      return state.specialFlags?.includes(rule)
        ? pass()
        : fail(`Special prerequisite requires explicit rule flag: ${rule}.`);
    }
  }
}

export function evaluateDnd35FeatPrerequisites(
  prerequisite: Dnd35Prerequisite | undefined,
  state: Dnd35FeatQualificationState,
): Dnd35PrerequisiteResult {
