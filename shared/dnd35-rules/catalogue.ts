...
export function listDnd35SpellsForClass(classId: string) {
  return DND35_SPELLS.flatMap((spell) => spell.classAccess.filter((access) => access.classId === classId).map((access) => ({ spell, level: access.level, tradition: access.tradition }))).sort((a, b) => a.level - b.level || a.spell.name.localeCompare(b.spell.name));
}

export function listDnd35SpellsForDomain(domainId: string) {
  return DND35_SPELLS.flatMap((spell) => (spell.domainAccess ?? []).filter((access) => access.domainId === domainId).map((access) => ({ spell, level: access.level }))).sort((a, b) => a.level - b.level || a.spell.name.localeCompare(b.spell.name));
}
...
export function searchDnd35Rules(query: string) {
  ...
  return {
    spells: DND35_SPELLS.filter((spell) => [spell.id, spell.name, spell.school, ...spell.tags, spell.rulesSummary].some((value) => normalize(value).includes(needle))),
    feats: DND35_CORE_FEATS.filter((feat) => [feat.id, feat.name, ...feat.categories, ...feat.tags, feat.rulesSummary].some((value) => normalize(value).includes(needle))),
  };
}
